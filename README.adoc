...
Alternatively, a Dockerfile is provided that builds an image that can build
MushMatch for you. If you run this in the MushMatch directory:

    ```console
    $ docker build .
    ```

This produces an image that, each time when run, will pull `master` from GitHub
and use that to build the very latest build of MushMatch.

The Docker tag for this image is `gustavo6046/alpine-ut99-mushmatch`.

This does not need a pre-existing UT install, as the Unreal Tournament server
assets and binaries are presumably not copyrighted; in any case, they are
hosted by UT-Files.com, and thus any possible liability falls upon them. The
server install is only used to build MushMatch using `ucc`.

The container will have a Unreal Tournament server in `/opt/ut-server`;
when built, MushMatch will have ZIPs made and ready for distribution at
`/opt/ut-server/Dist`.

Thus, to get the MushMatch zip file out of the container, one must perform this
command _after running the Docker container to build MushMatch_:

    ```console
    docker cp 'gustavo6046/alpine-ut99-mushmatch:/opt/ut-server/Dist/MushMatch/Latest/*' target/dir
    ```
