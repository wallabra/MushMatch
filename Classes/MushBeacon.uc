...
{    
    ...
    if ( IsInState('Pickup') || Owner == None || Owner.IsInState('Dying') || ( MushMatch(Level.Game) == none && Pawn(Owner).PlayerReplicationInfo != none && Pawn(Owner).PlayerReplicationInfo.Deaths > 0 ) )
    {
        Destroy();
        return;
    }
...
function float RateSelf(out int bUseAltMode)
{
    ...
    if ( BestPawn != None && Bot(Owner) != None )
